	...
	<div class="page-box">
		<ul>
			...
			<c:if test="${page.isPrev }">
				<li><a href="${cpath }/board/search/?type=${param.type}&search=${param.search}&reqPage=${page.begin - 1 }">⏪</a></li>
			</c:if>
			<c:if test="${param.reqPage > 1 }">
				<li><a href="${cpath }/board/search/?type=${param.type}&search=${param.search}&reqPage=${param.reqPage -1}">⬅</a></li>
			</c:if>
			...
			<!-- 페이지 항목 -->
		  	<c:forEach var="i" begin="${page.begin }" end="${page.end }">
		  		<c:choose>
			  		<c:when test="${param.reqPage == i }">
			  			<li><a style="color:red" href="${cpath }/board/search/?type=${param.type}&search=${param.search}&reqPage=${i } ">${i }</a></li>
			  		</c:when>
			  		<c:otherwise>
			  			<li><a style="color:blue" href="${cpath }/board/search/?type=${param.type}&search=${param.search}&reqPage=${i }">${i }</a></li>
			  		</c:otherwise>
		  		</c:choose>
		  	</c:forEach>
		  	...
		  	<c:if test="${page.isNext}">
				<li><a href="${cpath }/board/search/?type=${param.type}&search=${param.search}&reqPage=${page.end + 1 }">⏩</a></li>
			</c:if>
		</ul>
	</div>
